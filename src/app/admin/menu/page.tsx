...
export default function AdminMenuPage() {
  ...
  const loadItems = async () => {
    ...
    if (data.length > 0) { setItems(data); setCache('adminMenuItems', data); }
      ...
      const result = seeded.success ? seeded.data! : FALLBACK_MENU.map(item => ({ ...item, available: true }));
      setItems(result);
      setCache('adminMenuItems', result);
    }
  };
  ...
  useEffect(() => {
    checkAdminAuth().then(async (authed) => {
      ...
      setAuthorized(true);
      try { await loadItems(); } catch { setItems(FALLBACK_MENU.map(item => ({ ...item, available: true }))); }
    });
  }, []);
